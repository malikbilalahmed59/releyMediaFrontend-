...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/accounts/addresses/`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to fetch addresses: ${response.statusText}`, details: errorText },
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      return NextResponse.json(
        { error: `Failed to create address: ${response.statusText}`, details: errorText },
        { status: response.status }
      );
    }
