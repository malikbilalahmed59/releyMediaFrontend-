...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/accounts/token/refresh/`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Token refresh failed: ${response.statusText}`, details: errorText },
    ...
    console.error('Error proxying token refresh request:', error);
    return NextResponse.json(
